...
static void notifier_cb(struct lttng_session *session, void *priv)
{
	...
	if (statedump_done_file) {
		/* FIXME: Somebody may be waiting on this thread,
		 * how to tell it if file creation went wrong? */
...
int main(int argc, char **argv)
{
	...
	if (argc >= 2) {
		statedump_done_file = argv[1];
	}
	if (argc >= 3) {
		initialization_file = argv[2];
	}
	if (argc >= 4) {
		terminating_file = argv[3];
	}
	...
	if (terminating_file) {
		...
		/* Just sleep one second for the program to 'do' something */
		usleep_safe(1000000);
	}
