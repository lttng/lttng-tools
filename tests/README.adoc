...
// SPDX-FileCopyrightText: 2024-2025 Kienan Stewart <kstewart@efficios.com>
//
...
| `LTTNG_TEST_GDBSERVER_CONSUMERD_KERNEL
| Launch `gdbserver` and attach to newly spawned `lttng-consumerd` process
| {check} | {ex} | {ex}

| `LTTNG_TEST_GDBSERVER_CONSUMERD_KERNEL_PORT`
| The TCP port for the gdbserver instance. Default:{nbsp}`1026`
| {check} | {ex} | {ex}

| `LTTNG_TEST_GDBSERVER_CONSUMERD_KERNEL_WAIT`
| When not empty, the test script will wait for user input after starting lttng-consumerd before continuing.
Useful to connect and insert breakpoints before other commands are run.
| {check} | {ex} | {ex}

| `LTTNG_TEST_GDBSERVER_CONSUMERD_UST32`
| Launch `gdbserver` and attach to newly spawned `lttng-consumerd` UST32 process
| {check} | {ex} | {ex}

| `LTTNG_TEST_GDBSERVER_CONSUMERD_UST32_PORT`
| The TCP port for the gdbserver instance. Default:{nbsp}`1027`
| {check} | {ex} | {ex}

| `LTTNG_TEST_GDBSERVER_CONSUMERD_UST32_WAIT`
| When not empty, the test script will wait for user input after starting lttng-consumerd before continuing.
Useful to connect and insert breakpoints before other commands are run.
| {check} | {ex} | {ex}

| `LTTNG_TEST_GDBSERVER_CONSUMERD_UST64`
| Launch `gdbserver` and attach to newly spawned `lttng-consumerd` UST64 process
| {check} | {ex} | {ex}

| `LTTNG_TEST_GDBSERVER_CONSUMERD_UST64_PORT`
| The TCP port for the gdbserver instance. Default:{nbsp}`1028`
| {check} | {ex} | {ex}

| `LTTNG_TEST_GDBSERVER_CONSUMERD_UST64_WAIT`
| When not empty, the test script will wait for user input after starting lttng-consumerd before continuing.
Useful to connect and insert breakpoints before other commands are run.
| {check} | {ex} | {ex}
...
| `LTTNG_TEST_PRESERVE_TEST_ENV_DIR`
| Move the preserved test environments to the given path. The path must exist and be a directory.
| {check} | {ex} | {ex}

| `LTTNG_TEST_PRESERVE_TEST_ENV_ON_FAILURE`
| Set `LTTNG_TEST_PRESERVE_TEST_ENV` when the tap generator encounters a failure
| {check} | {ex} | {ex}
|===
