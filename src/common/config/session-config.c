		...
		xmlNodePtr attribute_node)
{
	...
	/* userspace probe location attributes. */
	for (function_attribute_node =
		...
		if (!strcmp((const char *) function_attribute_node->name,
			...
			// TODO should the DEFAULT lookup method be handle like
			// this? i.e. use ELF right away or keep the default
			// mention in the event? Is this information
			// useful?
		...
		xmlNodePtr attribute_node)
{
	...
	char *probe_name = NULL, *provider_name = NULL, *binary_path = NULL, *lookup_method_name;
	struct lttng_userspace_probe_location *location = NULL;
	struct lttng_userspace_probe_location_lookup_method *lookup_method = NULL;
