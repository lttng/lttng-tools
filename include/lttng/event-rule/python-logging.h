...
#ifndef LTTNG_EVENT_RULE_PYTHON_LOGGING_H
...
<table>
  ...
  <tr>
    ...
    <td>Any name
    <td>lttng_event_rule_python_logging_set_name_pattern()
  <tr>
...
@returns
    @parblock
    Python logging event rule on success, or \c NULL on error.
...
/*!
@brief
    Sets the event name pattern of the Python logging
    event rule \lt_p{rule} to \lt_p{pattern}.

See the \ref api-er-conds-event-name "event name" condition.

@param[in] rule
    Python logging event rule of which to set the event name pattern.
@param[in] pattern
...
@pre
    ...
    @lt_pre_has_type{rule,LTTNG_EVENT_RULE_TYPE_PYTHON_LOGGING}
    @lt_pre_not_null{pattern}
    @lt_pre_not_empty{pattern}

@sa lttng_event_rule_python_logging_get_name_pattern() --
    Get the event name pattern of a Python logging event rule.
*/
...
/*!
@brief
    Sets \lt_p{*pattern} to the event name pattern of the Python logging
    event rule \lt_p{rule}.

See the \ref api-er-conds-event-name "event name" condition.

@param[in] rule
    Python logging event rule of which to get the event name pattern.
@param[out] pattern
    @parblock
...
@sa lttng_event_rule_python_logging_set_name_pattern() --
    Set the event name pattern of a Python logging event rule.
*/
LTTNG_EXPORT extern enum lttng_event_rule_status
...
/*!
@brief
    Sets the event payload and context filter of the Python logging
...
@param[in] rule
    Python logging event rule of which to set the filter.
...
@sa lttng_event_rule_python_logging_get_filter() --
    Get the event payload and context filter of a
    Python logging event rule.
*/
LTTNG_EXPORT extern enum lttng_event_rule_status
lttng_event_rule_python_logging_set_filter(struct lttng_event_rule *rule, const char *filter_expr);

/*!
@brief
    Sets \lt_p{*filter_expr} to the event payload and context filter
    expression of the Python logging event rule \lt_p{rule}.

See the \ref api-er-conds-filter "event payload and context filter"
condition.

@param[in] rule
    Python logging event rule of which to get the
...
@sa lttng_event_rule_python_logging_set_filter() --
    Set the event payload and context filter of a
    Python logging event rule.
*/
LTTNG_EXPORT extern enum lttng_event_rule_status
...
/*!
@brief
    Sets the instrumentation point log level rule of the Python logging
...
@param[in] event_rule
    Python logging event rule of which to set the log level rule.
...
@pre
    ...
    @lt_pre_has_type{event_rule,LTTNG_EVENT_RULE_TYPE_PYTHON_LOGGING}
    @lt_pre_not_null{log_level_rule}

@sa lttng_event_rule_python_logging_get_log_level_rule() --
...
@brief
    ...
    log level rule of the Python logging event rule \lt_p{event_rule}.

See the \ref api-er-conds-ll "instrumentation point log level"
condition.

@param[in] event_rule
    Python logging event rule of which to get the log level rule.
...
@sa lttng_event_rule_python_logging_set_log_level_rule() --
...
#endif /* LTTNG_EVENT_RULE_PYTHON_LOGGING_H */
